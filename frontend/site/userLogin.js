const userLogin = function(resolve, reject) {
	getTemplate("userLogin.html").then(resp => {
		resolve({
			template: resp,
			data: function() {
				return {
			...
			methods: {
				handleLoginSubmit: function () {
                    ...
                    {
                        this.loading = true;
                        loadFromServer("/auth/user/login", this.handleloginResponse, "POST",
                        this.form.value);
                    }
				...
				handleloginResponse: function (resp) {
					...
					if (resp.status == 200) {		
						...
						this.$router.replace({name: "ecommerce"});
					}
